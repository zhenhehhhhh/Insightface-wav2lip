...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 调整
...
def bcmatting(location, blocation, btgt_bgr_dir, bgr_dir, bbgr_dir, tgt_bgr_dir):
    ...
    if step == 1:
        ...
        out = cv2.VideoWriter(save_dir + 'step1out.mp4', fourcc, 30, (bWidth, bHeight))
    ...
    elif btgt_bgr_dir[-3:] == 'jpg' or 'jpeg' or 'png' or 'gif':
    ...
    print('f:', str(fx1) + ':' + str(fy1) + ':' + str(fx2) + ':' + str(fy2))
    print('c:', str(cx1) + ':' + str(cy1) + ':' + str(cx2) + ':' + str(cy2))
    print('d:', str(dx) + ':' + str(dy) + ':' + str(ddx) + ':' + str(ddy))
    # ---- step2 加载背景图片 ---- #
    bbgr_img = cv2.imread(bbgr_dir)  # 加载背景图片
    bbgr_img = cv2.resize(bbgr_img, (blw, blh))
    bbgr_img = bbgr_img[cy1:cy2,cx1:cx2].copy()  # 边缘裁剪
    print('test1:', str(cy1) + ':' + str(cy2) + ':' + str(cx1) + ':' + str(cx2))
    cv2.imwrite('./temp/test2/'+'bc.jpg', bbgr_img)
    ...
    # ---- step2 输出视频保存设置 ---- #
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    out = cv2.VideoWriter(save_dir + 'step2out.mp4', fourcc, 30, (bbWidth, bbHeight))
    ...
    with torch.no_grad():
        for item in flist[:]:
            ...
            if btgt_bgr_dir[-3:] == 'mp4' or 'avi':
                ...
                btgt_img_bc = cv2.imread(btgt_img_bc)
            ...
            if ret:
                ...
                print(bsrc_frame.shape)
                print('test2:', str(cy1) + ':' + str(cy2) + ':' + str(cx1) + ':' + str(cx2))
                cv2.imwrite('./temp/test/'+str(count)+'.jpg', bsrc_frame)  #  测试！！！！！
                ...
                cv2.imwrite('./temp/test2/'+'fgr.jpg', fgr)  #  测试！！！！！

                pha[pha<0.6] = pha[pha<0.6]/2

                cv2.imwrite('./temp/test2/'+'pha.jpg', pha*255)  #  测试！！！！！
                # ---- 区域掩膜置零 ---- #
                # bc_part = (parse_img[:, :, 0] >= 250) & (parse_img[:, :, 1] == 0) & (parse_img[:, :, 2] == 0)  # 选取蓝色区域
                # pha[bc_part] = 0  # 

                btgt_bgr_img = btgt_img_bc[fy1:fy2, fx1:fx2].copy()
                cv2.imwrite('./temp/test2/'+'tar.jpg', btgt_bgr_img)  #  测试！！！！！
                btgt_bgr_img = cv2.resize(btgt_bgr_img, (ddx, ddy))
                print('fgr:',fgr.shape)
                print('pha:',pha.shape)
                print('btgt_bgr_img:',btgt_bgr_img.shape)
                com = fgr * pha + btgt_bgr_img * (1 - pha)
                com = cv2.resize(btgt_bgr_img, (dx, dy))  # 还原至原来的图像尺寸
                cv2.imwrite('./temp/test2/'+'com.jpg', com)  #  测试！！！！！
