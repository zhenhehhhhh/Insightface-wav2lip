...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        # source_image
        frame = cv2.imread('tools/chsy/00036740.jpg')
        ...
        frame_driving = cv2.imread('tools/chsy/kouxing/00000860.jpg')
        t1 = time.time()
        # 驱动视频人脸提取(替代DFM)
        ...
        merged_frame = Face_Merge(frame_raw, aligner_resolution, face_align_img, face_align_mask_img,
                                  face_swap_img, face_swap_mask_img, aligned_to_source_uni_mat, frame_width,
                                  frame_height,
        ...
        cv2.imwrite('tools/chsy/1.jpg', merged_frame.astype(np.uint8))
        # cv2.waitKey(1)
        t2 = time.time()
        print('fps:', 1 / (t2 - t1))
